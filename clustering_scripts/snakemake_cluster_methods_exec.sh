...
PARAMETER_FLAG=0 #flag to track if a parameter file was provided. If not, pipeline will fail because this is a required argument
...
while getopts ":hFNRi:s:o:d:m:p:e:" opt; do
	case $opt in
	h)
		echo -e "\nUsage: bash /path/to/snakemake_cluster_methods_exec.sh -m <Input_Manifest> -o <Output_Directory> ... <other options>\n"
	...
	p)
		...
		PARAMETER_FLAG=1
		echo "-p flag observed. PARAMETER_FILE set to ${OPTARG}." >&2
		;;
	e)
		EMAIL=${OPTARG}
		EMAIL_FLAG=1
...
if [ "$EMAIL_FLAG" == 1 ]; then
	...
	QSUB="--jobs 30 --cluster "\"'qsub -cwd -pe smp {threads} -l mem_free={resources.mem_qsub},h_vmem={resources.mem_qsub} -l h_rt={resources.job_time} -j yes -V -o ${LOG_DIR} -e ${LOG_DIR} -m bea -M ${EMAIL} -P corceslab'\"
else
	QSUB="--jobs 30 --cluster "\"'qsub -cwd -pe smp {threads} -l mem_free={resources.mem_qsub} -l h_rt={resources.job_time} -j yes -V -o ${LOG_DIR} -e ${LOG_DIR} -P corceslab'\"
fi
CLUSTER_SUB=${QSUB}
...
if [ "$PARAMETER_FLAG" != 1 ]; then
	echo "No parameter file provided. -p argument is required!"
	exit 1
fi
...
COMMAND="snakemake --rerun-incomplete --snakefile ${SCRIPT_DIR}/snakemake_cluster_methods.py --config in_dir=${INPUT_DIR} out_dir=${OUTPUT_DIR} \
temp_dir=${TEMP_DIR} scripts=${SCRIPT_DIR} metadata=${METADATA_FILE} parameters=${PARAMETER_FILE} --keep-going ${CLUSTER_SUB} --latency-wait 120"
